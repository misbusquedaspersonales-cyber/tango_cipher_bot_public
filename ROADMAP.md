...
**Web Share Target completado a nivel PWA/Chrome** ✅ — `pwa/manifest.json` ahora incluye `share_target` para archivos `.txt`. Service Worker maneja POST requests del share target y almacena archivos compartidos temporalmente en IndexedDB. Receptor puede compartir archivos .txt desde Telegram directamente con la app (flujo "un tap") en lugar del flujo manual anterior, cuando la app fue instalada vía Chrome.

**⚠️ Brecha detectada y parcheada:** el `.apk` (TWA) real que se distribuye para sideload (ver `PASOS_APK.md`) NO heredaba este cambio — `bubblewrap` solo lee `share_target` en el `bubblewrap init` inicial, y `tango-cifrado-apk/twa-manifest.json` ya existía (versionCode 3) desde antes de este cambio. Fix: `scripts/apk/sync-share-target.sh`, invocado automáticamente por `build-apk.sh` en cada build, sincroniza `share_target` hacia `twa-manifest.json` y corre `bubblewrap update` cuando hace falta regenerar `AndroidManifest.xml`. **Completado:** APK actualizado (versionCode 4 / 1.3.0) con Web Share Target funcional. Checklist `MOBILE_TESTING.md` §10.1.1 actualizado para validar sobre el `.apk` real.
