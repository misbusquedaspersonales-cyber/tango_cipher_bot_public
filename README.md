...
| `tests/` | 119 tests total once `private_core/` is populated: 53 JS (`tests/js/`) + 24 Python always-runnable (`test_build_encrypted_bundle.py` 5 + `test_telegram_client.py` 8 + `test_check_coverage.py` 11) + 43 Python requiring `private_core/` (`test_cipher_engine.py`: 33 original + 11 shared vectors). JS breakdown: `cipherEngine.test.mjs` 27 (16 fixed + 11 shared-vector loop), `pwa_e2e.test.mjs` 3, `deeplink.test.mjs` 23. Run `python3 -m pytest tests/python/ -v` and `npm test` separately. |
