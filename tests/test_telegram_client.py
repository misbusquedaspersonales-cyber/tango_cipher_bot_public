...
def test_enviar_mensaje_exitoso():
    ...
    with patch("telegram_client.requests.post", return_value=mock_resp) as mock_post:
...
def test_enviar_mensaje_fallo_http():
    ...
    with patch("telegram_client.requests.post", return_value=mock_resp):
        result = enviar_mensaje("TOKEN", "123", "hola")
...
def test_timeout_retorna_false():
    with patch("telegram_client.requests.post", side_effect=requests.exceptions.Timeout):
        assert enviar_mensaje("TOKEN", "123", "hola") is False


def test_connection_error_retorna_false():
    with patch("telegram_client.requests.post", side_effect=requests.exceptions.ConnectionError):
        assert enviar_mensaje("TOKEN", "123", "hola") is False


def test_request_exception_retorna_false():
    with patch("telegram_client.requests.post", side_effect=requests.exceptions.RequestException("fail")):
        assert enviar_mensaje("TOKEN", "123", "hola") is False
