...
- `pwa/service-worker.js:21` — `CACHE_VERSION` bump `tango-cifrado-v3 → v4 → v5`:
  - `v4` bump anterior por mobile styling.
  - `v5` (este release) fuerza purga de shell/bundle caches en todos los clientes ya instalados, porque se regeneró `pwa/encrypted-bundle.json` con una passphrase de despliegue nueva y no se quiere que ningún dispositivo pinneado a un bundle viejo siga recibiendo "clave incorrecta" por cache-first.
...
- **Deploy / producción**: curl sobre la URL pública de GitHub Pages confirmó que los 7 archivos críticos del shell de la PWA devuelven `HTTP 200` con `Content-Type` correcto y tamaño razonable, tanto con User-Agent desktop (`X11 Linux Chrome 120`) como con User-Agent móvil (`Android 14 Pixel 8 Chrome Mobile 120`):
  - `/pwa/index.html` — 200 / `text/html` / 11.4 KB
  - `/pwa/service-worker.js` — 200 / `application/javascript` / 3.4 KB  →  `CACHE_VERSION=tango-cifrado-v5`
...
- **drift-check.yml sintaxis**: validado con `yaml.safe_load` (Python 3.12) sin errores. Variables `${{ secrets.PRIVATE_REPO_PAT }}`, `${{ github.repository }}`, expresiones `if:` y step IDs todas alineadas con la sintaxis de GitHub Actions.
